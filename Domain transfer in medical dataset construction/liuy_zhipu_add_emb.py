    ...
    for node in node_label_names:
        ...
        if len(data)>0:
            ...
            for data_inrow in data:
                prompt= prompt+f"label为{data_inrow[0]}的节点\n"  
    if found:
        return prompt 
    else:
        return "" 
...
key_values = None
with open("zhipu_query_pair_query.json", 'r',encoding="utf-8") as json_file:
...
with open('new_query_pair_train_zhipu.json', 'w',encoding="utf-8") as json_file:
